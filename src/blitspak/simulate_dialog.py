...
Ui_ScrutinizeDialog, QDialog = loadUiType('..\\..\\Resources\\UI\\simulate_dialog.ui')

class SimulateDialog(widgets.QDialog, Ui_ScrutinizeDialog):
    ...
    def __init__(self, parent):
        ...
        self.cmb_experiment.currentIndexChanged.connect(self.on_current_experiment_changed)
        ...
        self.fill_experiment_library()
        self.current_experiment = ""
        self.x_outer_limits = {}
        self.param_values_fit, self.conf_intervals_fit, self.dw_statistic_fit = {}, {}, {}
        
        # Prepare the UI
        self.cmb_experiment.setSizeAdjustPolicy(widgets.QComboBox.AdjustToContents)
        for i in self.available_functions:
            name = self.fn_names[i]
            self.cmb_experiment.addItem(name)
...
#             self.full_data[key].index = range(len(self.full_data[key]))
        ...
        self.ui_ready = True
#         self.on_current_experiment_changed() # Sets up the tables
...
#         return np.unique(anx)
    ...
    def fill_experiment_library(self):                
        for name in self.fn_dictionary:
            ...
            self.experiment_library[name] = ff.ModellingFunction(name, fn_ref, p0_fn_ref, param_names, fn_str)    
    ...
    def get_param_values_from_table(self, series_names):
        ...
        funcname = self.cmb_experiment_function.currentText()
    ...
    def get_constant_params_from_table(self, series_names):
        ...
        funcname = self.cmb_experiment.currentText()
        param_names = list(self.fn_dictionary[funcname][self.d_pnames])
    ...
    def on_calc(self):
        func = self.experiment_library[self.current_experiment].fn_ref
...
#         self.set_tbl_param_values(p0s)
    ...
    def on_current_experiment_changed(self):
        if self.ui_ready:
            self.txt_experiment.clear()
            self.current_experiment = self.cmb_experiment.currentText()
            self.txt_experiment.setText(self.experiment_library[self.current_experiment].fn_str)
            self.txt_experiment.adjustSize()
    ...
    def get_hor_header_params_table(self, global_fit):
        ...
        if self.current_experiment != "":
            param_names = self.experiment_library[self.current_experiment].param_names 
    ...
    def get_p0s(self):
        p0_func = self.experiment_library[self.current_experiment].p0_fn_ref
        p_names = self.experiment_library[self.current_experiment].param_names
        param_values = {}
        series_names = self.get_selected_series_names()
        data = self.get_data_for_fitting(series_names)
        i = 0
